function dYdx = MOL_deriv_rad(x,Y)
...
l_ghostPoint = Y(l + i - 1) + ((Y(l + i)*Y(R)*2*xi_step)/Sigma_l)*(...
                    ...
                    1/Y(m + i)*(k*h_n*(n_inf + Y(m + i) + Y(l + i) - 1))/(Y(l + i) + (1 - Y(l + i))*(1 - 1/Y(m + i)))...
               ) + ...
...
dYdx(m + i, 1) = (6/(xi_step^2*Y(R)^2))*(...
                 ...
                 - Y(m + i)*rad_strength*heaviside(x - rad_start)*exp(-rad_dur*(x - rad_start));
...
dYdx(i + a, 1) = (6/(xi_step^2*Y(R)^2))*(...
                    (Y(a + i + 1) - Y(a + i))...
                 ) + d_a*(fun_p_a(Y(l + i),Y(m + i),Y(c + i)) - Y(a + i)) + gamma*Y(m + i)*rad_strength*heaviside(x - rad_start)*exp(-rad_dur*(x - rad_start));
...
for i = 2:1/xi_step 
    ...
    dYdx(i + l, 1) = (xi_value(i)/Y(R))*DRdt*(Y(l + i + 1) - Y(l + i - 1))/(2*xi_step)  + ...
                     (1/(k*Y(R)^2))*(...
                        (2/xi_value(i))*(...
                            Sigma_l*(Y(l + i + 1) - Y(l + i -1))/(2*xi_step) - chi_l*((1 - Y(l + i)))*Y(l + i)*(Y(a + i + 1) - Y(a + i -1))/(2*xi_step) - Y(l + i)*(Sigma_m*(Y(m + i + 1) - Y(m + i - 1))/(2*xi_step) - Sigma_l*Y(l + i)*(Y(l + i + 1) - Y(l + i - 1))/(2*xi_step))...
                        ) + ...
...
dYdx(i + l, 1) = (xi_value(i)/Y(R))*DRdt*(l_ghostPoint - Y(l + i - 1))/(2*xi_step)  + ...
                     (1/Y(R)^2)*(...
                        (2/(k*xi_value(i)))*(...
                            Sigma_l*(l_ghostPoint - Y(l + i -1))/(2*xi_step) - chi_l*(1 - Y(l + i))*Y(l + i)*(a_ghostPoint - Y(a + i - 1))/(2*xi_step) - Y(l + i)*(Sigma_m*(m_ghostPoint - Y(m + i - 1))/(2*xi_step) - Sigma_l*Y(l + i)*(l_ghostPoint - Y(l + i - 1))/(2*xi_step))...
                        ) + ...
